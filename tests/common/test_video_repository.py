...
async def test_create_and_get_by_id_round_trip(db):
    video = _make_video()

    video_repository.create(db, video)
    await db.commit()

    fetched = await video_repository.get_by_id(db, video.id)
...
async def test_get_by_id_returns_none_for_unknown_id(db):
    assert await video_repository.get_by_id(db, uuid.uuid4()) is None
