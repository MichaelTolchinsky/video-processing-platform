...
def get_by_video_and_type(
    db: Session, video_id: uuid.UUID, asset_type: AssetType
) -> GeneratedAsset | None:
    return db.execute(
...
def list_for_video(db: Session, video_id: uuid.UUID) -> list[GeneratedAsset]:
    return list(
...
def create(db: Session, asset: GeneratedAsset) -> None:
    db.add(asset)
